import * as React from "react";
import type { SVGProps } from "react";
const FirebaseLight = (props: SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    xmlnsXlink="http://www.w3.org/1999/xlink"
    width="1em"
    height="1em"
    fill="none"
    viewBox="0 0 256 256"
    {...props}
  >
    <rect width={256} height={256} fill="#F4F2ED" rx={60} />
    <mask
      id="prefix__a"
      width={146}
      height={200}
      x={55}
      y={28}
      maskUnits="userSpaceOnUse"
      style={{
        maskType: "alpha",
      }}
    >
      <path
        fill="#fff"
        d="M182.035 72.67a4.58 4.58 0 0 0-3.115-3.597 4.6 4.6 0 0 0-4.64 1.097l-23.605 23.748-18.702-35.338a4.45 4.45 0 0 0-3.991-2.472 4.45 4.45 0 0 0-3.991 2.472l-10.149 19.317L88.184 30.4c-2.052-3.863-7.868-2.84-8.552 1.478L55 189.366l66.026 36.929a13.9 13.9 0 0 0 13.342 0l66.596-36.929z"
      />
    </mask>
    <g mask="url(#prefix__a)">
      <path
        fill="url(#prefix__b)"
        d="M182.035 72.67a4.58 4.58 0 0 0-3.115-3.597 4.6 4.6 0 0 0-4.64 1.097l-23.605 23.748-18.702-35.338a4.45 4.45 0 0 0-3.991-2.472 4.45 4.45 0 0 0-3.991 2.472l-10.149 19.317L88.184 30.4c-2.052-3.863-7.868-2.84-8.552 1.478L55 189.366l66.026 36.929a13.9 13.9 0 0 0 13.342 0l66.596-36.929z"
      />
      <path
        fill="#FFA000"
        d="M55 189.366 79.746 31.877c.684-4.317 6.5-5.34 8.552-1.477l25.544 47.497 10.149-19.317a4.56 4.56 0 0 1 4.048-2.45c1.703 0 3.264.944 4.048 2.45l68.877 130.786z"
      />
      <path fill="url(#prefix__c)" d="M45.877 67.784h99.21v126.127h-99.21z" />
      <path fill="#F57C00" d="m140.298 128.007-26.57-50.11L55 189.366z" />
      <path fill="url(#prefix__d)" d="M50.439 58.694h159.649v174.988H50.439z" />
      <path
        fill="#FFCA28"
        d="M200.965 189.366 182.035 72.67a4.58 4.58 0 0 0-3.115-3.597 4.6 4.6 0 0 0-4.639 1.097L55.001 189.366l66.026 36.93a13.91 13.91 0 0 0 13.342 0z"
      />
      <path
        fill="#fff"
        fillOpacity={0.2}
        d="M182.035 72.67a4.58 4.58 0 0 0-3.115-3.597 4.6 4.6 0 0 0-4.639 1.097l-23.606 23.748-18.701-35.338a4.45 4.45 0 0 0-3.991-2.472 4.45 4.45 0 0 0-3.992 2.472l-10.149 19.317L88.184 30.4c-2.052-3.863-7.868-2.84-8.552 1.478L55 189.366h-.114l.114.114.684.341 94.877-94.766 23.72-23.635a4.6 4.6 0 0 1 4.639-1.097 4.58 4.58 0 0 1 3.115 3.597l18.702 115.56.228-.114zM55.228 189.139 79.746 33.014c.684-4.318 6.5-5.34 8.552-1.477l25.544 47.496 10.149-19.317a4.57 4.57 0 0 1 4.049-2.45c1.702 0 3.263.945 4.048 2.45l18.245 34.543z"
      />
      <path
        fill="#A52714"
        d="M134.368 225.159a13.9 13.9 0 0 1-13.342 0l-65.912-36.815-.114 1.022 66.026 36.929a13.9 13.9 0 0 0 13.342 0l66.596-36.929-.114-1.022z"
        opacity={0.2}
      />
      <path
        fill="url(#prefix__e)"
        d="M182.035 72.67a4.58 4.58 0 0 0-3.115-3.597 4.6 4.6 0 0 0-4.64 1.097l-23.605 23.748-18.702-35.338a4.45 4.45 0 0 0-3.991-2.472 4.45 4.45 0 0 0-3.991 2.472l-10.149 19.317L88.184 30.4c-2.052-3.863-7.868-2.84-8.552 1.478L55 189.366l66.026 36.929a13.9 13.9 0 0 0 13.342 0l66.596-36.929z"
      />
    </g>
    <defs>
      <linearGradient
        id="prefix__b"
        x1={49.918}
        x2={231.544}
        y1={62.053}
        y2={158.45}
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#fff" stopOpacity={0.1} />
        <stop offset={1} stopColor="#fff" stopOpacity={0} />
      </linearGradient>
      <linearGradient
        id="prefix__e"
        x1={49.918}
        x2={231.544}
        y1={62.053}
        y2={158.45}
        gradientUnits="userSpaceOnUse"
      >
        <stop stopColor="#fff" stopOpacity={0.1} />
        <stop offset={1} stopColor="#fff" stopOpacity={0} />
      </linearGradient>
      <pattern
        id="prefix__c"
        width={1}
        height={1}
        patternContentUnits="objectBoundingBox"
      >
        <use xlinkHref="#prefix__f" transform="scale(.0115 .00893)" />
      </pattern>
      <pattern
        id="prefix__d"
        width={1}
        height={1}
        patternContentUnits="objectBoundingBox"
      >
        <use xlinkHref="#prefix__g" transform="scale(.00714 .00645)" />
      </pattern>
      <image
        xlinkHref="data:image/png;base64,"
        id="prefix__f"
        width={87}
        height={112}
      />
      <image
        xlinkHref="data:image/png;base64,"
        id="prefix__g"
        width={140}
        height={155}
      />
    </defs>
  </svg>
);
export { FirebaseLight };
